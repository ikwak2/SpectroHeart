...
def feature_extract_bound_melspec(data, samp_sec=20, sr = 4000, pre_emphasis = 0, hop_length=256, win_length = 512, n_mels = 100):
    ...
    if samp_sec:
        if len(data) > sr * samp_sec :
            n_samp = len(data) // int(sr * samp_sec)
            signal = []
            for i in range(n_samp) :
                signal.append(data[ int(sr * samp_sec)*i:(int(sr * samp_sec)*(i+1))])
        else :
            n_samp = 1
            signal = np.zeros(int(sr*samp_sec,))
            for i in range(int(sr * samp_sec) // len(data)) :
                signal[(i)*len(data):(i+1)*len(data)] = data
            num_last = int(sr * samp_sec) - len(data)*(i+1)
            signal[(i+1)*len(data):int(sr * samp_sec)] = data[:num_last]
            signal = [signal]
    else:
        n_samp = 1
        signal = [data]
    ...
    for i in range(num_patient_files):
        ...
        for j in range(num_locations) :
            ...
            if use_raw :
                frequency1, recording1 = sp.io.wavfile.read(filename)
                nr = len(recording1)
                if len(recording1) >= use_raw : 
                    recording1 = recording1[:use_raw]
                else :
                    recording1 = np.pad(recording1, (0, use_raw - len(recording1) ), constant_values=(0,0) )
            else :
...
#            if use_raw :
#                current_mm_labels2 = np.zeros(768)
#                current_out_labels2 = np.zeros(768)
#                lc1 = int(nr / 246000 *768)
#                if mm_label == 'Absent' :
#                    current_mm_labels2[:lc1] = 2
#                elif mm_label == 'Unknown' :
#                    current_mm_labels2[:lc1] = 2
#                else :
#                    current_mm_labels2[:lc1] = 1
#            else :
#                current_mm_labels2 = np.zeros(0)
#                current_out_labels2 = np.zeros(0)
#            mm_labels2.append(current_mm_labels2)            
#            out_labels2.append(current_out_labels2)                   
...
def get_features_3lb_all_ord_rr(data_folder, patient_files_trn, po = .3,
                          samp_sec=20, pre_emphasis = 0, n_mels = 100,hop_length=256, win_length = 512,
                                hop_length1=256, win_length1 = 512, hop_length2=256, win_length2 = 512, hop_length3=256, win_length3 = 512,
                             filter_scale = 1, n_bins = 80, fmin = 10, trim = 4000,
    ...
    for i in range(num_patient_files):
        ...
        for j in range(num_locations) :
            ...
            # Extract melspec
            if use_mel :
                mel1 = feature_extract_melspec(filename, samp_sec=samp_sec, pre_emphasis = pre_emphasis, hop_length=hop_length1, 
                                               win_length = win_length1, n_mels = n_mels, trim = trim)[0]
                mel2 = feature_extract_melspec(filename, samp_sec=samp_sec, pre_emphasis = pre_emphasis, hop_length=hop_length2, 
                                               win_length = win_length2, n_mels = n_mels, trim = trim)[0]
                mel3 = feature_extract_melspec(filename, samp_sec=samp_sec, pre_emphasis = pre_emphasis, hop_length=hop_length3, 
                                               win_length = win_length3, n_mels = n_mels, trim = trim)[0]
            else :
                mel1 = np.zeros( (1,1,1) )
                mel2 = np.zeros( (1,1,1) )
                mel3 = np.zeros( (1,1,1) )
            ...
            features['mel1'].append(mel1)
            features['mel2'].append(mel2)
            features['mel3'].append(mel3)
            
            
            if use_cqt :
                cqt = feature_extract_cqt(filename, samp_sec=samp_sec, pre_emphasis = pre_emphasis, filter_scale = filter_scale, 
                                           n_bins = n_bins, fmin = fmin, trim = trim)[0]
            else :
                cqt = np.zeros( (1,1,1) )
            features['cqt1'].append(cqt)

            if use_stft :
                stft = feature_extract_stft(filename, samp_sec=samp_sec, pre_emphasis = pre_emphasis, hop_length=hop_length, 
                                            win_length = win_length, trim = trim)[0]
            else :
                stft = np.zeros( (1,1,1) )
            features['stft1'].append(stft)
...
#            out_labels2.append(current_out_labels2)                   
    ...
    if use_mel : 
        ...
        M, N, _ = features['mel1'][i].shape
    mel_input_shape1 = (M,N,1)
        
    # mel2 #    
    if use_mel : 
        M, N = features['mel2'][i].shape
        for i in range(len(features['mel2'])) :
            features['mel2'][i] = features['mel2'][i].reshape(M,N,1)
        print("melspec2: ", M,N)
    else :
        M, N, _ = features['mel2'][i].shape
    mel_input_shape2 = (M,N,1)
    
    # mel3 #
    if use_mel : 
        M, N = features['mel3'][i].shape
        for i in range(len(features['mel3'])) :
            features['mel3'][i] = features['mel3'][i].reshape(M,N,1)
        print("melspec3: ", M,N)
    else :
        M, N, _ = features['mel3'][i].shape
    mel_input_shape3 = (M,N,1)
    ...
    mm_labels = np.array(mm_labels)
    out_labels = np.array(out_labels)
    return features, mm_labels, out_labels, mel_input_shape1, mel_input_shape2, mel_input_shape3, cqt_input_shape, stft_input_shape



def get_features_copy2(data_folder, patient_files_trn, po = .3,
                          samp_sec=20, pre_emphasis = 0, hop_length=256, win_length = 512, n_mels = 100,
                             filter_scale = 1, n_bins = 80, fmin = 10, trim = 4000,
                                use_mel = True, use_cqt = False, use_stft = False, use_raw = False, use_rr = False, use_rr_seq = False
                         ) :
    features = dict()
    features['id'] = []
    features['age'] = []
...
#             else :
#                 info = 0
#             features['rr_seq0'].append(info)
#             num = features['rr_seq0'][i]['ECG_R_Peaks']
#             features['rr_seq1'].append(num)
...
def get_features_6(data_folder, patient_files_trn, po = .3,
                          samp_sec=20, pre_emphasis = 0, n_mels = 100,
                                hop_length1=256, win_length1 = 512, hop_length2=256, win_length2 = 512, hop_length3=256, win_length3 = 512,
                             filter_scale = 1, n_bins = 80, fmin = 10, trim = 4000,
                                use_mel = True, use_cqt = False, use_stft = False, use_raw = False, use_rr = False
                         ) :
...
# FHS and Murmus extraction proces 4~6
# ------------------------------------
# description: Feature extraction: extract boundary detect and signal envelope 
# ------------------------------------
def preprocess_signal(amplit_smooth):
    mu = np.mean(amplit_smooth)
    var = np.var(amplit_smooth)
    t_sh = mu + var + 0.05 # mu + var + epsilon

    thres_list = np.where(amplit_smooth > t_sh)[0]

    packet = []
    tmp = []
    if thres_list.size > 0:
        v = thres_list[0]
        tmp.append(v)

    for vv in thres_list[1:]:
        if v + 1 == vv:
            tmp.append(vv)
            v = vv
        else:
    ...
    min_list = [min(p) for p in packet if p]
    max_list = [max(p) for p in packet if p]

    new_list = []
    for i in range(len(max_list) - 1):
        if abs(max_list[i] - min_list[i + 1]) < 60:
            new_list.extend(packet[i] + packet[i + 1])

    final_list = set(new_list)

    drop_list = [0 if i in final_list else p for i, p in enumerate(packet)]

    seq_remake = drop_list + new_list
    li = [i for i in seq_remake if i != 0]

    min_list1 = [min(packet[p]) for p in range(len(packet)) if packet[p]]
    max_list1 = [max(packet[p]) for p in range(len(packet)) if packet[p]]

    # Feature Extraction: S1 and S2
    s_detect = amplit_smooth.copy()
    for i in range(len(max_list1) - 1):
        s_detect[:min_list1[0]] = 0
        s_detect[max_list1[i]:min_list1[i + 1]] = 0

    s1s2_detect = pad_sequences([s_detect], maxlen=80000, dtype='float64', padding='post', truncating='post', value=0.0)
    s1s2_mel = feature_extract_melspec(s_detect)[0] # apply melspec

    # Feature Extraction: systolic and diastolic murmurs 
    mm_detect = amplit_smooth.copy()
    for i in range(len(max_list1)):
        mm_detect[min_list1[i]:max_list1[i] + 1] = 0

    murmur_detect = pad_sequences([mm_detect], maxlen=80000, dtype='float64', padding='post', truncating='post', value=0.0)
    murmur_mel = feature_extract_melspec(mm_detect)[0] # apply melspec

    # Feature Extraction: Sinal envelope
    envelope = amplit_smooth.copy()
    envel_mel = feature_extract_melspec(envelope)[0] # apply melspec

    return s1s2_detect, murmur_detect, envelope, s1s2_mel, murmur_mel, envel_mel
